...
void IR::SetResultsDisplay(std::string &results_top, std::string &results_table)
{
    ...
    results_table = "Party, Final Votes, Final Vote Percentage\n"; 
    for (int idx = 0; idx < n_candidates; idx++)
    {
        results_table += candidates[idx].name;
        results_table += ",";
        results_table += std::to_string(candidates[idx].GetVoteCount());
        results_table += ",";
        results_table += std::to_string(100*candidates[idx].GetVoteCount()/n_ballots);
        results_table += "%\n";
    }
};
