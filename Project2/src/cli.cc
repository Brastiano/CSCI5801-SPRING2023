...
void Cli::SetLayout1()
{
    ...
    std::string prompt_2 =  "\nPlease enter the name of the election results file to load.\n"
                            ...
                            "(.txt, .csv, and so on).\n\n";
    std::string prompt_3 = "Do you have additional ballot files to input? (Enter y or n)\n\n";
    bool status;
    ...
    std::cin >> add_files;
    if (add_files == "y" || add_files == "Y" || add_files == "yes")
    {
        std::string filenames[10];
        filenames[0] = filename;
        int idx = 1;
        while(SetLayout1a(filenames, idx))
        {
            idx++;
        }
        status = controller_->ReadElectionType(filenames, date, idx+1);
    }
    else
    {
        std::string filenames[1] = {filename};
        status = controller_->ReadElectionType(filenames, date, 1);
    }
    /**
...
bool Cli::SetLayout1a(std::string filenames[], int idx)
{
    std::string layout_1_header =   "#=========================#\n"
                                    "# Election File Selection #\n"
                                    "#=========================#\n\n";
    ...
    ClearConsole();
    std::cout << layout_1_header << prompt_2;
    std::cin >> filenames[idx];
    std::cout << prompt_3;
    std::string add_files = "";
    std::cin >> add_files;
    if (add_files == "y" || add_files == "Y" || add_files == "yes")
...
void Cli::SetLayout2(   std::string election_type, std::string num_candidates,
                        std::string candidates, std::string ballot_quantity, std::string filenames[], int n_files)
{
    std::string layout_2_file_header =  "#======================#\n"

                                        "#    File Information  #\n"
                                        "#======================#\n\n";
    std::string layout_2_files = "";
    for (int idx = 0; idx<n_files; idx++)
    {
        layout_2_files += filenames[idx];
        layout_2_files += "\n";
    }
    ...
    ClearConsole();
    std::cout << layout_2_file_header << layout_2_files << layout_2_header << header << std::endl;
    std::cout << prompt_3;
    std::string response;
    std::cin >> response;
...
void Cli::SetLayout2(   std::string election_type, std::string num_parties,
                        std::string parties, std::string seat_quantity, std::string ballot_quantity, std::string filenames[], int n_files)
{
    std::string layout_2_file_header =  "#======================#\n"
                                        "#    File Information  #\n"
                                        "#======================#\n\n";
